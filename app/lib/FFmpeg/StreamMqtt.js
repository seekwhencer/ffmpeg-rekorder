export default class FFmpegStreamMqtt extends MODULECLASS {
    ...
    publish(topic, value) {
        ...
        LOG(this.label, this.stream.name, 'PUBLISH', topic, value);
    ...
    listenMqttControls() {
        ...
        if (this.mqttControlTopicEnable)
            APP.MQTT.on(this.mqttControlTopicEnable, data => {
                LOG(this.label, this.stream.name, 'GOT MESSAGE:', data, 'ON', this.mqttControlTopicEnable);
        ...
        if (this.mqttControlTopicRecord)
            APP.MQTT.on(this.mqttControlTopicRecord, data => {
                LOG(this.label, this.stream.name, 'GOT MESSAGE:', data, 'ON', this.mqttControlTopicRecord);
        ...
        if (this.mqttControlOptionsTopic)
            ['property'].forEach(option => {
                APP.MQTT.on(`${this.mqttControlOptionsTopic}/${option}`, data => {
                    LOG(this.label, this.stream.name, 'GOT MESSAGE:', data, 'ON', `${this.mqttControlOptionsTopic}/${option}`);
